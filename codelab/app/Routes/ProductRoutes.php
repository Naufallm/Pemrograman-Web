...
class ProductRoutes {
    public function handle($method, $path) {
        ...
        if ($method == "GET" && $path == "/web_modul4/codelab/index.php/api/product") {
            $controller = new ProductController();
            echo $controller->index();
        }

        // JIKA REQUEST METHOD GET DAN PATH SAMA DENGAN '/api/product/{id}'
        if ($method == "GET" && strpos($path, "/web_modul4/codelab/index.php/api/product/") === 0) {
        ...
        if ($method == "POST" && $path == "/web_modul4/codelab/index.php/api/product") {
            $controller = new ProductController();
            echo $controller->insert();
        }

        // JIKA REQUEST METHOD PUT DAN PATH SAMA DENGAN '/api/product/{id}'
        if ($method == "PUT" && strpos($path, "/web_modul4/codelab/index.php/api/product/") === 0) {
            $pathParts = explode("/", $path);
            $id = $pathParts[count($pathParts) - 1];
        ...
        if ($method == "DELETE" && strpos($path, "/web_modul4/codelab/index.php/api/product/") === 0) {
            $pathParts = explode("/", $path);
            $id = $pathParts[count($pathParts) - 1];
